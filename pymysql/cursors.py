...
class Cursor(object):
    ...
    def executemany(self, query, seq_of_args):
        ...
        re_insert = re.compile(r'insert\s+into', re.I)
        re_insert_values = insert_values
        if re.match(re_insert, query):
            re_comment = re.compile(r"\/\*.*\*\/")
            re_on_duplicate = re.compile(r'\s*on duplicate key.*$', re.I)
            # remove:
            # 1. comments
            # 2. everything after 'on duplicate key'
            # as these might cause re_insert_values to match
            # more than it has to
            tmp = re.sub(re_on_duplicate, '', re.sub(re_comment, '', query))
            matches = re.search(re_insert_values, tmp)
            if not matches:
